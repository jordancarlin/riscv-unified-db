...
class TestYamlLoader < Minitest::Test
    ...
    Dir.mktmpdir do |dir|
      ...
      stdout, stderr, status =
        Dir.chdir(Udb.repo_root) do
          Open3.capture3("/bin/bash -c \"source /opt/venv/bin/activate && /opt/venv/bin/python3 #{UDB_GEM_PATH}/python/yaml_resolver.py resolve --no-progress --no-checks #{arch_dir} #{resolved_dir}\"")
        end
      # puts stdout
      # puts stderr
      # puts status
  ...
  def resolve_multi_yaml(*yamls)
    Dir.mktmpdir do |dir|
      ...
      system "/bin/bash -c \"source /opt/venv/bin/activate && /opt/venv/bin/python3 #{UDB_GEM_PATH}/python/yaml_resolver.py resolve --no-checks #{arch_dir} #{resolved_dir}\""
