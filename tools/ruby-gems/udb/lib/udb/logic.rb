    ...
    def <=>(other)
      ...
      if name != other_param.name
        ...
        if cv.is_a?(String)
          ...
          other_cv = other_param.comparison_value
          raise "Expected boolean comparison_value, got #{other_cv.class}" unless other_cv == true || other_cv == false
          (cv ? 1 : 0) <=> (other_cv ? 1 : 0) # <=> not defined for booleans, so convert to integer
        else
